...
export class RedisDeserializer implements IRedisDeserializer {
  ...
  private parseValue(): RespType {
    ...
    console.log(`Parsing token ${JSON.stringify(token)} at position ${this.pos}`);
    ...
    switch (token) {
      ...
      default:
        throw new Error(`Invalid token ${JSON.stringify(token)} at ${this.pos}`);
    }
  }
  ...
  private parseBulkStrings(): string | null {
    ...
    console.log(`Length indicator: ${length}`);
    ...
    while (i < length && this.pos < this.inputLength) {
      console.log(`Parsing character at position ${this.pos}: ${this.getCurrentToken()}`);
      str += this.getCurrentToken();
      this.consumeToken();
      i++;
    }
